...
char* getSimilar(char st1[max_size], float *M, char *vocab, long long words,
...
void server(float *M, char *vocab, long long words, long long size,
	...
	while (1) {
		...
		while (1) {
			...
			std::string tmp(st1);
			// std::cout << tmp.substr(0, tmp.size()-1);
			char st2[max_size]; 
			strcpy(st2,tmp.substr(0,tmp.size()-1).c_str());
			// std::cout << st2;
			// do your evil bidding here
			char* ans = getSimilar(st2, M, vocab, words, size, bestw);
			if (strcmp(ans, "-1") == 0) {
